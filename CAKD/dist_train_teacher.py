"""
Script huấn luyện mô hình Giáo Viên (Teacher) ViT-B/16 trên tập dữ liệu tự chọn.
Dùng Transfer Learning: Tải trọng số ImageNet pretrained, thay lớp head cuối cùng
thành num_classes của dataset, rồi fine-tune toàn bộ mạng.

Sau khi huấn luyện xong, file checkpoint sẽ được lưu tại --output-dir,
và được dùng làm đầu vào cho dist_train_cakd.py (--teacher-checkpoint).
"""
import datetime
import os
import time
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            # Modified ViT returns (logits, attn_weights, token, feat)
        ...
        acc1, acc5 = new_utils.accuracy(output, target, topk=(1, min(4, len(data_loader.dataset.classes))))
...
def evaluate(model, criterion, data_loader, device, print_freq=100, log_suffix=""):
    ...
    with torch.inference_mode():
        for image, target in metric_logger.log_every(data_loader, print_freq, header):
            ...
            acc1, acc5 = new_utils.accuracy(output, target, topk=(1, min(4, output.shape[1])))
            batch_size = image.shape[0]
...
def main(args):
    ...
    print("Creating ViT-B/16 Teacher model")
    print(f"  -> Number of classes detected: {num_classes}")

    # ===== PHẦN QUAN TRỌNG: Transfer Learning cho ViT-B/16 =====
    # 1. Tải mạng ViT-B/16 cùng trọng số ImageNet pretrained (1000 classes)
    model = torchvision.models.vit_b_16(weights=ViT_B_16_Weights.IMAGENET1K_V1)

    # 2. Thay thế lớp phân loại cuối cùng (head) từ 1000 -> num_classes
    #    ViT-B/16 có hidden_dim = 768
    model.heads.head = nn.Linear(model.heads.head.in_features, num_classes)
    print(f"  -> Replaced head: 1000 classes -> {num_classes} classes")
    ...
    print("Start training Teacher (ViT-B/16)")
    start_time = time.time()
    best_acc = 0.0
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            # Lưu checkpoint tốt nhất (best accuracy)
            if acc > best_acc:
                best_acc = acc
                new_utils.save_on_master(checkpoint, os.path.join(args.output_dir, "best_teacher.pth"))
                print(f"  -> Saved best teacher checkpoint with Acc@1 = {best_acc:.3f}")
    ...
    print(f"Best Teacher Acc@1: {best_acc:.3f}")
    print(f"Best teacher checkpoint saved at: {os.path.join(args.output_dir, 'best_teacher.pth')}")


def get_args_parser(add_help=True):
    import argparse

    parser = argparse.ArgumentParser(description="PyTorch ViT-B/16 Teacher Training (Fine-tune)", add_help=add_help)
